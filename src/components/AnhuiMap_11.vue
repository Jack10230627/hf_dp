...
export default {
  ...
  methods: {
    ...
    createLakes(lakeJson, demMax) {
      ...
      // ✨ 流动水面 Shader（明亮 + 涟漪 + Fresnel + 波光）
      this.lakeMaterial = new THREE.ShaderMaterial({
        uniforms: {
          uTime:         { value: 0 },
          uDeepColor:    { value: new THREE.Color(0x0a3a7a) },
          uShallowColor: { value: new THREE.Color(0x3aaff0) },
          uHighlight:    { value: new THREE.Color(0xaaeeff) },
        ...
        fragmentShader: `
          ...
          void main() {
            // 多层方向相反的流动波（横向流动，避免放射）
            vec2 flow1 = vUv * 5.0  + vec2(uTime * 0.08, uTime * 0.05);
            vec2 flow2 = vUv * 10.0 - vec2(uTime * 0.06, uTime * 0.10);
            float w1 = fbm(flow1);
            float w2 = fbm(flow2);
            ...
            float ripple = sin(d * 40.0 - uTime * 2.0) * 0.5 + 0.5;
            ripple *= smoothstep(0.5, 0.0, d);
            col += uHighlight * ripple * 0.08;

            // Fresnel 边缘反射
            vec3 viewDir = normalize(uCameraPos - vWorldPos);
            float fres = pow(1.0 - max(dot(vNormal, viewDir), 0.0), 2.5);
            col += uHighlight * fres * 0.5;

            // 高频波光闪烁
            float sparkle = smoothstep(0.6, 0.75, w2) * smoothstep(0.55, 0.85, w1);
            col += uHighlight * sparkle * 1.2;

            // 流动高光带
            float band = sin(vUv.y * 25.0 + waves * 6.0 + uTime * 0.8);
            band = smoothstep(0.7, 1.0, band);
            col += uHighlight * band * 0.25;

            // 边缘略暗（湖岸阴影）
            float edgeShadow = smoothstep(0.5, 0.42, d);
            col *= mix(0.75, 1.0, edgeShadow);
      ...
      lakeJson.features.forEach(feature => {
        ...
        polygons.forEach(polygon => {
          ...
          ring.forEach((coord, i) => {
            ...
            ptsXZ.push({ x, z })
            // 用原始高度（未挖坑）确定岸边水位
            maxOriginalH = Math.max(maxOriginalH, this.getOriginalH(u, v, demMax))
            // 用挖坑后的高度确定湖底
            minDepressedH = Math.min(minDepressedH, this.getH(u, v, demMax))
          })

          // 水面位置：略低于岸边原始高度，让水"刚好填满凹陷"
          const waterLevel = maxOriginalH - 5
          // 湖底位置：在凹陷底部之上一点点
          const bedLevel = Math.min(minDepressedH + 10, waterLevel - 30)
          ...
          bedMesh.renderOrder = 9
          this.mapGroup.add(bedMesh)

          // 2️⃣ 水面（流动 Shader，盖在凹陷之上）
          ...
          // 3️⃣ 岸边发光描边（贴在水面边缘）
          const edgePts = ptsXZ.map(p => new THREE.Vector3(p.x, waterLevel + 2, p.z))
          if (edgePts.length > 2) {
            const curve = new THREE.CatmullRomCurve3(edgePts, true)
            const edgeGeom = new THREE.TubeGeometry(curve, edgePts.length * 2, 5, 6, true)
          ...
          if (name && name !== "湖泊" && !labeledLakes.has(name)) {
            this.createRiverLabel(name, new THREE.Vector3(centroidX, waterLevel + 80, centroidZ))
