...
vec3 brownHigh = vec3(0.85, 0.65, 0.25);
      ...
      mesh.position.y = baseHeight + 2   // ✅ 抬高避免冲突
      this.scene.add(mesh)
  ...
  `,
    ...
    void main(){
      ...
      float heightFactor = clamp(vPos.y / 700.0, 0.0, 1.0);

      vec3 topGold = vec3(0.85,0.70,0.40);
      vec3 bottomGold = vec3(0.45,0.30,0.15);

      vec3 color = mix(bottomGold, topGold, heightFactor);

      float fresnel = pow(1.0 - abs(vNormal.y), 2.0);
      color += fresnel * 0.25;
