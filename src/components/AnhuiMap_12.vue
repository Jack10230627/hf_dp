  ...
  methods: {
    ...
    getH(u, v, demMax) {
      ...
      if (this.lakeMaskData) {
        ...
        if (mask < 0.5) {
          // 越接近 0（湖中心），下沉越深
          const sinkFactor = (0.5 - mask) * 2
          h -= sinkFactor * this.config.lakeDepth
        }
      }
      return h
    },
      ...
      riverJson.features.forEach(f => {
        ...
        lines.forEach(line => {
          ...
          const mat = new THREE.ShaderMaterial({
            uniforms: {
              uTime: { value: 0 },
              uColor: { value: name === "淮河" ? new THREE.Color(0x00ffff) : new THREE.Color(0x0088ff) }
            },
        ...
        if (allSegmentPoints.length > 0 && name !== "未命名" && !labeledRivers.has(name)) {
          ...
          if (bestPos) {
            this.createRiverLabel(name, bestPos)
    ...
    createLakes(lakeJson, demMax) {
      ...
      // ✨ 水面 Shader（降亮 + 流动 + 涟漪 + Fresnel）
      this.lakeMaterial = new THREE.ShaderMaterial({
        uniforms: {
          uTime:         { value: 0 },
          uDeepColor:    { value: new THREE.Color(0x041d4a) },
          uShallowColor: { value: new THREE.Color(0x1976d2) },
          uHighlight:    { value: new THREE.Color(0x5fc8ff) },
        ...
        fragmentShader: `
          uniform float uTime;
          uniform vec3 uDeepColor, uShallowColor, uHighlight, uCameraPos;
          varying vec2 vUv;
          varying vec3 vWorldPos;
          varying vec3 vNormal;
          ...
          void main() {
            vec2 flow1 = vUv * 5.0  + vec2(uTime * 0.06, uTime * 0.04);
            vec2 flow2 = vUv * 12.0 - vec2(uTime * 0.05, uTime * 0.08);
            float w1 = fbm(flow1);
            float w2 = fbm(flow2);
            ...
            float ripple = sin(d * 35.0 - uTime * 1.8) * 0.5 + 0.5;
            ...
            col += uHighlight * fres * 0.15;

            // 波光闪烁（弱）
            float sparkle = smoothstep(0.65, 0.78, w2) * smoothstep(0.6, 0.85, w1);
            col += uHighlight * sparkle * 0.4;

            // 流动条带（很弱）
            float band = sin(vUv.y * 30.0 + waves * 5.0 + uTime * 0.6);
            band = smoothstep(0.85, 1.0, band);
            col += uHighlight * band * 0.1;

            // ✨ 关键：钳制颜色，防止 Bloom 吹爆
            col = min(col, vec3(0.7, 0.85, 1.0));
      ...
      // 湖底材质（极深色）
      const lakeBedMaterial = new THREE.MeshBasicMaterial({
        color: 0x010816,
        transparent: false
      })

      // 湖岸侧壁材质（凹陷的墙壁）
      const lakeWallMaterial = new THREE.MeshBasicMaterial({
        color: 0x0a2040,
        side: THREE.DoubleSide
      })
      ...
      lakeJson.features.forEach(feature => {
        ...
        polygons.forEach(polygon => {
          ...
          ring.forEach((coord, i) => {
            ...
            ptsXZ.push({ x, z })
            sumH += this.getOriginalH(u, v, demMax)
          })

          const avgH = sumH / ring.length

          // ✨ 三层高度（关键：水面低于岸边）
          const shoreLevel = avgH + 80         // 岸顶
          const waterLevel = avgH + 20         // 水面（比岸顶低 60）
          const bedLevel   = avgH - 60         // 湖底（比水面再低 80）
          ...
          const buildGeom = () => {
            ...
            for (let i = 0; i < uvAttr.count; i++) {
              const px = g.attributes.position.getX(i)
          ...
          // 1️⃣ 湖底
          const bedMesh = new THREE.Mesh(buildGeom(), lakeBedMaterial)
          bedMesh.rotateX(Math.PI / 2)
          bedMesh.position.y = bedLevel
          bedMesh.renderOrder = 8
          this.mapGroup.add(bedMesh)

          // 2️⃣ ✨ 湖岸侧壁（垂直墙壁，呈现"碗"形）
          const wallVertices = []
          const wallIndices = []
          for (let i = 0; i < ptsXZ.length; i++) {
            const p = ptsXZ[i]
            wallVertices.push(p.x, shoreLevel, p.z)     // 上沿
            wallVertices.push(p.x, bedLevel, p.z)       // 下沿
          }
          for (let i = 0; i < ptsXZ.length; i++) {
            const a = i * 2
            const b = i * 2 + 1
            const c = ((i + 1) % ptsXZ.length) * 2
            const d = ((i + 1) % ptsXZ.length) * 2 + 1
          ...
          const wallMesh = new THREE.Mesh(wallGeom, lakeWallMaterial)
          ...
          // 4️⃣ 岸顶发光描边
          const edgePts = ptsXZ.map(p => new THREE.Vector3(p.x, shoreLevel + 2, p.z))
          if (edgePts.length > 2) {
            const curve = new THREE.CatmullRomCurve3(edgePts, true)
            const edgeGeom = new THREE.TubeGeometry(curve, edgePts.length * 2, 4, 6, true)
          ...
          if (name && name !== "湖泊" && !labeledLakes.has(name)) {
            this.createRiverLabel(name, new THREE.Vector3(centroidX, shoreLevel + 100, centroidZ))
