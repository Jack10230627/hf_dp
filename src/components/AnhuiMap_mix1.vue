  ...
  methods: {
    ...
    createRivers(riverJson, demMax) {
      ...
      riverJson.features.forEach((f) => {
        ...
        const radius =
          name === "淮河" || name === "长江"
            ? this.config.riverBaseWidth * 5
            : this.config.riverBaseWidth * 1.5;
    ...
    animate() {
      ...
      // 1. 如果正在过渡中，直接跳过所有逻辑，只负责渲染，防止控制冲突
      ...
      if (this.camera && this.controls) {
        ...
        // 触发 2D 切换
        if (dist < 5000) {
          this.isTransitioning = true; // 立即锁死，防止重复触发
          this.controls.enabled = false; // 立即禁用鼠标控制，防止抖动

          const lonLat = this.threePosToLonLat(this.controls.target);

          // 执行一个微小的向内俯冲动画，增强视觉联系，然后再通知父组件
          gsap.to(this.camera.position, {
            x: this.controls.target.x,
            y: this.controls.target.y + 1000,
            z: this.controls.target.z + 1000,
            duration: 0.6,
            onComplete: () => {
              this.$emit("zoom-in-limit", { lonLat });
            },
          });
        }
      }
    ...
    syncViewFrom2D(lonLat) {
      this.isTransitioning = true; // 锁死
      this.controls.enabled = false;

      const targetPos = this.lonLatToThreePos(lonLat);
      // 相机先瞬间定位到目标点上空
      this.camera.position.set(targetPos.x, 3000, targetPos.z + 3000);
      ...
      gsap.to(this.camera.position, {
        ...
        ease: "power2.out",
        onComplete: () => {
          this.isTransitioning = false; // 解锁
          this.controls.enabled = true; // 恢复鼠标控制
        },
      });
    },
    onResize() {
