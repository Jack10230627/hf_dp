...
export default {
  ...
  data() {
    return {
      ...
      config: {
        width: 20000,
        height: 20000,
        maxH: 1100,        // 750 → 1000（山更高更明显）
        baseY: -400,
        hExp: 1.2,         // 1.7 → 1.3（不要压太平，让山有起伏）
        segments: 512,
        wallStep: 6,
        sink: 50
      },
  ...
  void main(){
    ...
    // 更鲜明的色阶：增强饱和度
    vec3 c0 = vec3(0.04, 0.10, 0.25);    // 深蓝（更深）
    vec3 c1 = vec3(0.05, 0.18, 0.38);    // 暗蓝
    vec3 c2 = vec3(0.00, 0.60, 0.80);    // 亮青
    vec3 c3 = vec3(0.15, 0.55, 0.35);    // 翠绿（更深更饱和）
    vec3 c4 = vec3(0.40, 0.70, 0.25);    // 鲜绿（黄绿过渡）
    vec3 c5 = vec3(0.85, 0.75, 0.30);    // 黄色
    vec3 c6 = vec3(0.85, 0.55, 0.15);    // 橙黄
    vec3 c7 = vec3(0.65, 0.30, 0.10);    // 棕红
    vec3 c8 = vec3(0.45, 0.20, 0.10);    // 深棕
...
`
    ...
    async loadRivers() {
      try {
        ...
        Object.keys(riverGroups).forEach(name => {
          console.log(`  ${name}: ${riverGroups[name].length} 段`)
        })
    ...
    createRivers(riverGroups) {
      ...
      Object.keys(riverGroups).forEach(name => {
        ...
        const allSegmentPoints = []  // 保存所有段的点（每段是一个数组）
    ...
    /**
     * 寻找最佳标签位置
     * 策略：在所有段中找到最长的那一段，返回它的「弧长中点」
     * 这样标签一定在河上、靠近视觉中心、避免弯曲处
     */
